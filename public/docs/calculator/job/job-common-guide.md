...
- **공통 하단 컴포넌트**: 모든 직장 계산기는 최하단에 `FAQ`와 `JobMoreCalculators` 컴포넌트를 반드시 포함하여 UI 일관성을 유지해야 함.

## 5. 입력 및 계산 가이드
- **날짜 등 특수 입력**: 날짜 선택기, 단위 변환기 등은 모바일에서 터치가 쉽도록 충분한 영역을 확보해야 함.
