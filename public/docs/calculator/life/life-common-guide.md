# JIKO 생활 계산기 공통 개발 가이드

신규 생활 계산기 메뉴를 추가할 때 준수해야 할 아키텍처 및 공통 기능 가이드입니다.

## 1. 폴더 및 파일 구조
모든 새 메뉴는 아래와 같은 구조를 따릅니다.
```text
app/calculator/life/[menu-name]/
...
### 4.2 탭 기반 내비게이션 (Tab-based Navigation)
- 서로 연관된 기능을 가진 계산기(예: 날짜 ↔ 디데이 ↔ 전역일)는 상단에 탭을 배치하여 사용자가 메뉴를 나가지 않고도 이동할 수 있도록 구성합니다.
- **컴포넌트**: `app/calculator/components/CalculatorTabs.tsx` (예정) 또는 공통 탭 스타일 준수.
- **일관성**: 직장 계산기의 '연봉/월급' 및 '실수령액' 탭과 동일한 디자인 톤앤매너를 유지합니다.
...
- **공통 하단 컴포넌트**: 모든 생활 계산기는 최하단에 `FAQ`와 `LifeMoreCalculators` 컴포넌트를 반드시 포함하여 UI 일관성을 유지해야 함.
