...
export async function generateStaticParams() {
    return productsData.loans.map(p => ({ slug: p.slug.normalize('NFC') }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
    const { slug } = await params;
    const normalizedSlug = decodeURIComponent(slug).normalize('NFC');
    const product = productsData.loans.find(p => p.slug.normalize('NFC') === normalizedSlug);
    if (!product) return {};
...
export default async function LoanSlugPage({ params }: Props) {
    ...
    const breadcrumbLd = generateBreadcrumbJsonLd([
        ...
        { name: product.name, item: `/calculator/finance/loans/${product.slug}` }
    ]);
    ...
    return (
        <main className="min-h-screen bg-gray-50 dark:bg-gray-900">
            ...
            <NavBar title={`${product.name} 대출 이자 계산기`} description={`${product.name} 대출 금액, 이자율, 상환 방식을 선택하여 월 상환액과 총 대출 이자를 정확하게 계산하세요. 거치 기간 설정을 지원합니다.`} position="top" />
