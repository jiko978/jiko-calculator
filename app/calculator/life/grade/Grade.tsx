...
export default function Grade() {
    ...
    // 변환기 상태용 리스트
    const convScaleList = ["4.5", "4.3", "4.0", "100"];
    ...
    return (
        <div className="max-w-3xl mx-auto px-4 py-8 pb-safe w-full">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-[32px] p-6 sm:p-8 shadow-xl border border-gray-100 dark:border-gray-700/50 space-y-8 transition-all duration-300">
                <div className="flex bg-gray-100 dark:bg-gray-900/50 p-1.5 rounded-2xl">
                    <button
                        onClick={() => { setActiveTab("calc"); setConvResult(null); setResult(null); }}
                    ...
                    <button
                        onClick={() => { setActiveTab("conv"); setConvResult(null); setResult(null); }}
                        className={`flex-1 py-3 text-center text-sm font-bold rounded-xl transition-all duration-200 ${
                            activeTab === "conv"
                                ? "bg-white dark:bg-gray-800 text-blue-600 shadow-sm ring-1 ring-black/5"
                                : "text-gray-500 hover:text-gray-700 dark:text-gray-400"
                ...
                {activeTab === "calc" && (
                    <div className="space-y-8 animate-fade-in">
                        <div>
                            ...
                            <div className="flex bg-gray-50 dark:bg-gray-900/50 p-1.5 rounded-xl border border-gray-100 dark:border-gray-700/50">
                        ...
                        <button
                            ...
                            className="w-full mt-4 py-4 border-2 border-dashed border-gray-200 dark:border-gray-700/50 rounded-2xl text-gray-400 font-bold hover:bg-gray-50 dark:hover:bg-gray-900/50 hover:text-blue-500 transition-all flex items-center justify-center gap-2"
                        >
                        ...
                        <div>
                            ...
                            <div className="flex bg-gray-50 dark:bg-gray-900/50 p-1.5 rounded-xl border border-gray-100 dark:border-gray-700/50">
                                {convScaleList.map((s) => (
                                    <button
                                        key={s}
                                        onClick={() => { setConvScale(s as ScaleType); setConvResult(null); }}
                                        className={`flex-1 py-2 text-xs font-black rounded-lg transition-all ${
                                            convScale === s 
                                            ? "bg-white dark:bg-gray-800 text-blue-600 shadow-sm ring-1 ring-black/5" 
                                            : "text-gray-400 hover:text-gray-600"
                        ...
                        <div>
                            ...
                            <div className="relative group">
                                <input
                                    ...
                                    placeholder="예: 3.8"
                                    value={convGpa}
                                    onChange={(e) => { setConvGpa(e.target.value); setConvResult(null); }}
                                    className="w-full h-16 bg-gray-50 dark:bg-gray-900/50 border-2 border-gray-100 dark:border-gray-700 rounded-2xl px-4 text-2xl font-black text-center focus:outline-none focus:border-blue-500 transition-all"
                                />
                                <div className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 font-bold text-sm">점</div>
                            </div>
                        </div>
                    </div>
                )}
                ...
                {hasError && (
                    <div className="bg-red-50 dark:bg-red-900/20 text-red-500 text-xs font-bold p-4 rounded-xl text-center border border-red-100 dark:border-red-800 animate-pulse">
                        🚨 {activeTab === "calc" ? "필수 입력 항목(학점/과목 등)을 확인해 주세요." : "변환할 학점을 정확히 입력해 주세요."}
                    </div>
                )}
            ...
            {activeTab === "calc" && result && (
                <div id="result-section" ref={resultRef} className="mt-8 bg-white dark:bg-gray-800 p-8 rounded-[32px] shadow-xl border border-gray-100 dark:border-gray-700/50 relative overflow-hidden animate-fade-slide-up space-y-8">
                    ...
                    <div className="text-center">
                        ...
                        <p className="text-sm font-bold text-gray-500 dark:text-gray-400">나의 학점 분석 리포트</p>
                    </div>
                    
                    <div className="bg-gradient-to-br from-blue-600 to-indigo-800 rounded-[24px] p-8 text-white shadow-lg text-center relative overflow-hidden">
                        <div className="absolute -right-10 -top-10 text-9xl opacity-10">🎓</div>
                        <p className="text-blue-100 font-bold mb-2 opacity-90 tracking-tight text-center">총 평점</p>
                        <div className="flex items-baseline justify-center gap-2">
                            <span className="text-6xl md:text-7xl font-black tracking-tighter">{result.totalGpa.toFixed(2)}</span>
                            <span className="text-2xl text-blue-200 font-black">/ {scale}</span>
                        </div>
                    </div>
            ...
            {activeTab === "conv" && convResult && (
                <div id="result-section" ref={resultRef} className="mt-8 bg-white dark:bg-gray-800 p-8 rounded-[32px] shadow-xl border border-gray-100 dark:border-gray-700/50 relative overflow-hidden animate-fade-slide-up space-y-8">
                    ...
                    <div className="text-center">
                        <h2 className="text-xl font-black text-gray-900 dark:text-white mb-2 flex items-center justify-center gap-2">
                            <span className="text-indigo-500">✨</span> 변환 결과
                        </h2>
                        <p className="text-sm font-bold text-gray-500 dark:text-gray-400">대학별 만점 기준 환산 결과</p>
                    </div>
